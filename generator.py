...
# 🔑 Токен бота-генератора
TOKEN = "8577015225:AAFbVE3hZ23HZI50gWk7d7vdgqi5rKHcJ4A"
bot = telebot.TeleBot(TOKEN)

# 🚫 Доступ только для создателя (укажите ваш Telegram ID)
CREATOR_ID = 7316276135

# 📋 Состояния диалога
user_state = {}   # {chat_id: шаг}
user_data = {}    # {chat_id: {business_name, token, admin_id, dev_id}}
...
# -------------------- ГЕНЕРАЦИЯ ФАЙЛА --------------------
def generate_custom_code(business_name, token, admin_id, dev_id):
    """
    Шаблон оригинального Bakebydi.py с заменами:
    {business_name} – название бизнеса
    {token} – токен телеграм-бота
    {admin_id} – ID администратора (кондитера)
    {dev_id} – ID разработчика (для бэкапов)
    ВАЖНО: оригинальная логика полностью сохранена, ничего не вырезано.
    """
    return f'''import sys
import subprocess
import os
...
TOKEN = "{token}"
DEV_ID = {dev_id}
ADMIN_ID = {admin_id}  # ID кондитера (админа)
...
create_dummy_image('hello.png', '{{business_name}} Welcome!')
create_dummy_image('Ahello.png', 'Admin Panel {{business_name}}')
...
def send_backup():
    try:
        ...
        # Архивируем
        shutil.make_archive('{business_name.replace(" ", "_")}_Backup', 'zip', backup_dir)
        
        # Отправляем
        with open('{business_name.replace(" ", "_")}_Backup.zip', 'rb') as f:
            bot.send_document(DEV_ID, f, caption="📦 Ежедневный бэкап (БД, Логи, Фото)")
        
        # Убираем за собой
        shutil.rmtree(backup_dir)
        os.remove('{business_name.replace(" ", "_")}_Backup.zip')
...
def send_welcome(message):
    ...
    if uid == ADMIN_ID:
        with open('Ahello.png', 'rb') as photo:
            bot.send_photo(uid, photo, caption="Приветствую, Кондитер! Панель управления {business_name} активирована.", reply_markup=get_admin_keyboard())
    else:
        # Обычный пользователь
        with open('hello.png', 'rb') as photo:
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🍰 Начать сборку заказа", callback_data="start_order"))
            bot.send_photo(uid, photo, caption="Добро пожаловать в {business_name}! Самые вкусные десерты здесь 🧁", reply_markup=markup)
...
if __name__ == '__main__':
    print("Бот {business_name} запущен!")
    bot.infinity_polling()
'''

# -------------------- ОБРАБОТЧИКИ КОМАНД ГЕНЕРАТОРА --------------------
@bot.message_handler(commands=['start'])
def start_generator(message):
    ...
    # Начинаем опрос с первого шага
    user_state[message.chat.id] = STEP_BUSINESS
    user_data[message.chat.id] = {}
    bot.send_message(message.chat.id, 
        "👋 Привет! Я помогу создать готового Telegram-бота на основе твоего шаблона.\n\n"
        "Шаг 1/4: Напиши **название бизнеса** (например: 'BakebyDI', 'FlowerShop', 'PizzaLand')."
    )

@bot.message_handler(func=lambda m: m.chat.id == CREATOR_ID and user_state.get(m.chat.id) == STEP_BUSINESS)
def get_business_name(message):
    if not message.text.strip():
        bot.send_message(message.chat.id, "Название не может быть пустым. Попробуй ещё раз.")
        return
    user_data[message.chat.id]['business_name'] = message.text.strip()
    user_state[message.chat.id] = STEP_TOKEN
    bot.send_message(message.chat.id,
        "✅ Принято.\n\n"
        "Шаг 2/4: Введи **токен будущего бота** (получить у @BotFather)."
    )

@bot.message_handler(func=lambda m: m.chat.id == CREATOR_ID and user_state.get(m.chat.id) == STEP_TOKEN)
def get_token(message):
    token = message.text.strip()
    if ':' not in token or len(token) < 30:
        bot.send_message(message.chat.id, "❌ Это не похоже на токен. Убедись, что скопировал его правильно.")
        return
    user_data[message.chat.id]['token'] = token
    user_state[message.chat.id] = STEP_ADMIN
    bot.send_message(message.chat.id,
        "✅ Токен сохранён.\n\n"
        "Шаг 3/4: Введи **Telegram ID администратора** (того, кто будет управлять заказами, добавлять товары).\n"
        "Это число, например: 7316276135"
    )

@bot.message_handler(func=lambda m: m.chat.id == CREATOR_ID and user_state.get(m.chat.id) == STEP_ADMIN)
def get_admin_id(message):
    try:
        ...
        return
    user_data[message.chat.id]['admin_id'] = admin_id
    user_state[message.chat.id] = STEP_DEV
    bot.send_message(message.chat.id,
        "✅ Админ ID записан.\n\n"
        "Шаг 4/4: Введи **Telegram ID разработчика** (кому будут приходить ежедневные бэкапы)."
    )

@bot.message_handler(func=lambda m: m.chat.id == CREATOR_ID and user_state.get(m.chat.id) == STEP_DEV)
def get_dev_id(message):
    try:
        dev_id = int(message.text.strip())
    except:
        bot.send_message(message.chat.id, "❌ Введи целое число.")
        return
    user_data[message.chat.id]['dev_id'] = dev_id
    # Генерация
    bot.send_message(message.chat.id, "⚙️ Собираю твоего бота, подожди пару секунд...")
    ...
    filename = f"{data['business_name'].replace(' ', '_')}_bot.py"
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(code)
    # Отправляем файл
    with open(filename, 'rb') as f:
        bot.send_document(message.chat.id, f, caption="🎉 Готово! Вот твой кастомный бот.\n\n"
                         "Загрузи этот файл на Railway (или другой хостинг), установи зависимости и запусти.")
    # Очистка состояния
    del user_state[message.chat.id]
    del user_data[message.chat.id]
    logging.info(f"Сгенерирован бот {data['business_name']} для пользователя {message.chat.id}")

@bot.message_handler(commands=['restart'])
def restart_generator(message):
    if message.chat.id == CREATOR_ID:
...
# Запуск бота-генератора
if __name__ == '__main__':
    print("Бот-генератор запущен. Доступен только создателю.")
    bot.infinity_polling()